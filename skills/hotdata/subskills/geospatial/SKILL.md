...
- Discover candidate tables/columns with **`hotdata databases tables list`** (filter with `--schema`/`--table`) or **`hotdata databases tables list`** (tables inside a managed database) — see core skill.
...
Find these columns by their `Binary` / `Struct` types in `hotdata databases tables list` (or `hotdata databases tables show <catalog.schema.table>`).
...
1. **Find geometry columns** — `hotdata databases tables list`; look for `Binary` (WKB) / `Struct` (bbox) types.
